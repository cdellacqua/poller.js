...
| `consumer` | (`data`: `T`) => `void` \| `Promise`<`void`\> | A consumer that will handle the received data. |
| `errorHandler?` | (`err`: `unknown`) => `void` \| `Promise`<`void`\> | (optional) An error handler that will receive the errors that can occur in the data producer or in the data consumer. |
| `monotonicTimeProvider?` | () => `number` | (optional, defaults to performance.now) A function that provides the current time, used to determine compute the dynamic interval. |
| `producer` | () => `T` \| `Promise`<`T`\> | The polling subject. |
